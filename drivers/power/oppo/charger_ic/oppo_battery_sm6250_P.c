...
#include "oppo_battery_sm6250_P.h"
...
#ifdef ODM_LQ_EDIT
bool charger_type_is_ocp(void)
{
	if (charger_type_ocp == 1)
		return true;
	else
		return false;
}
...
#ifdef VENDOR_EDIT
	/* Wei.Jun@BSP.CHG.Basic,charging_bsp 2019/09/12,Add for charging_bsp */
...
#ifdef ODM_LQ_EDIT
/* liujiawei@ODM_LQ.BSP.CHG, 2019/12/13,Add for PMIC SHIP MODE */
	...
	if (!chg->qc2_unsupported_voltage &&
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't turn off SUSPEND_ON_COLLAPSE_USBIN_BIT rc=%d\n",
					rc);
#ifdef VENDOR_EDIT
/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2019/11/20,Add for cdp D+ pull up charging_bsp */
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		/*Jun.Wei@RM.BSP.CHG.Basic, 2019/09/26, float & sdp rerun apsd for charger type detect error*/
		//if (reg_value & (CDP_CHARGER_BIT /*| SDP_CHARGER_BIT*/)) {
		if (reg_value & (FLOAT_CHARGER_BIT | SDP_CHARGER_BIT)) {
			chg->uusb_apsd_rerun_done = true;
			smblib_rerun_apsd(chg);
			return IRQ_HANDLED;
		}
...
/* Yichun.Chen  PSW.BSP.CHG  2019-05-30  for limit otg current */
	#ifndef ODM_LQ_EDIT
	/* Hongbin.Chen  ODM_LQ.BSP.CHG  2019-11-18  for limit otg current */
...
#ifdef VENDOR_EDIT
        /* Jun.Wei  PSW.BSP.CHG  2019-07-16  for CTS */
...
static int oppochg_usbtemp_monitor_main(void *data)
{
	...
	/*Jun.Wei@RM.BSP.CHG.Basic, 2019/07/22, 19771 two_ntc always */
	//if (cmp_pcb("EVT2") < 0 || high_temp_version == true) {
	if (high_temp_version == true) {
		chg_err("version not support: %d %d\n", cmp_pcb("EVT2"), high_temp_version);
		return 0;
	}
...
static void oppochg_clear_suspend(void)
{
	...
	#ifdef VENDOR_EDIT
	/* WeiJun@BSP.CHG.Basic,charging_bsp 2019/09/12,Add for charging_bsp */
	smblib_err(chg, "usbtemp test usb_status=%d\n", chg->usb_status);
	if(!chg->usb_status)
		rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 0);
	#endif /* VENDOR_EDIT */
...
aicl_return:
	...
	/*Jun.Wei@RM.BSP.CHG.Basic, 2019/09/18, add for pd led_on charging*/
	if (chip->pmic_spmi.smb5_chip->chg.pd_active) {
		rc = vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, PD_VOTER, true, usb_icl[i] * 1000);
	}
	// add end
...
//Jun.Wei@BSP.POWER.Basic 2019/07/09 add for pd charge 
#ifdef VENDOR_EDIT
int oppochg_get_charger_subtype(void)
{
	struct smb_charger *chg = NULL;
...
static int smbchg_pd_setup(void)
    //static void smbchg_pd_setup(int mV, int mA)
{
...
struct oppo_chg_operations oppochg_smb5_ops = {
	...
	.get_charger_subtype = oppochg_get_charger_subtype,
