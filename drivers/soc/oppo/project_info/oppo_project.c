...
#ifdef VENDOR_EDIT
//Zuming.Long@cn.nfc.basic, 2019.11.12,add for make oppo_info a global var
...
static int __init update_feature(void)
{
    mm_segment_t fs;
    fs = get_fs();
    pr_err("update_feature, Project Name [%d]", get_Operator_Version());
    set_fs(KERNEL_DS);
    if (oppo_info) {
        if (get_Operator_Version() == 26) {
...
bool is_confidential(void)
{
    ...
    return g_project->nEngVerConfidential;
}
...
bool oppo_daily_build(void)
{
    ...
    if ((ALL_NET_CMCC_TEST == g_project->nEngVersion) || (ALL_NET_CMCC_FIELD == g_project->nEngVersion) ||
        (ALL_NET_CU_TEST == g_project->nEngVersion) || (ALL_NET_CU_FIELD == g_project->nEngVersion) ||
        (ALL_NET_CT_TEST == g_project->nEngVersion) || (ALL_NET_CT_FIELD == g_project->nEngVersion))
        return false;
...
static int __init oppo_project_init(void)
{
    ...
    #ifdef VENDOR_EDIT
    //Zuming.Long@cn.nfc.basic, 2019.11.12,remove for make oppo_info a global var
    //static struct proc_dir_entry *oppo_info = NULL;
    #endif /* VENDOR_EDIT */
...
/* Cong.Dai@psw.bsp.tp, 2019.06.15, add for init engineering version file*/
	p_entry = proc_create("engVersion", S_IRUGO, oppo_info, &eng_version_proc_fops);
	if (!p_entry) {
			pr_err("create engVersion proc failed.\n");
			goto error_init;
	}
	p_entry = proc_create("isConfidential", S_IRUGO, oppo_info, &confidential_proc_fops);
	if (!p_entry) {
