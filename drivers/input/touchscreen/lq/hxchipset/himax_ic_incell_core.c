		...
		uint8_t storage_type, bool is_dirly)
{
	...
	do {
		...
		I("%s:data[3]=0x%2X,data[2]=0x%2X,data[1]=0x%2X,data[0]=0x%2X\n",
			__func__,
			back_data[3],
			back_data[2],
			back_data[1],
...
static void himax_mcu_idle_mode(int disable)
{
	...
	do {
		...
		I("%s:Mode[0]=0x%2X,Mode[1]=0x%2X,Mode[2]=0x%2X,Mode[3]=0x%2X\n",
			__func__,
			tmp_data[0],
			tmp_data[1],
			tmp_data[2],
...
#ifdef ODM_LQ_EDIT
	sprintf(ic_data->vendor_ic_fw_ver, "%d", ic_data->vendor_config_ver);
	I("CFG_VER : %s\n",ic_data->vendor_ic_fw_ver);
#endif
...
/* CORE_FW */
	...
	himax_parse_assign_cmd(fw_addr_trigger_addr,
...
/* CORE_DRIVER */
#if defined(HX_ZERO_FLASH)
	himax_parse_assign_cmd(zf_addr_dis_flash_reload,
	...
	himax_parse_assign_cmd(zf_data_sram_clean,
