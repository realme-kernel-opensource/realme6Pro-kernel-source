...
char *i_CTPM_firmware_name = "tp/19721/Himax_firmware.bin";
...
OPPO_proc_node_init(void)
{
	if( strcmp( HIMAX_PROC_TOUCHPANEL_FOLDER, HIMAX_PROC_TOUCH_FOLDER)!= 0  ){
		...
		I("%s: hiamx node is different to oppo.\n", __func__);
	}else{
		himax_proc_touchpanel_dir=himax_touch_proc_dir;
	}
...
OPPO_proc_node_deinit(void)
{
	...
	if( strcmp( HIMAX_PROC_TOUCHPANEL_FOLDER, HIMAX_PROC_TOUCH_FOLDER)!= 0  ){
	remove_proc_entry(HIMAX_PROC_TOUCHPANEL_FOLDER, NULL);	
	I("%s: hiamx node is not same with oppo.\n", __func__);
	}
...
#if defined(__EMBEDDED_FW__)
		image = &g_embedded_fw;
		I("%s:No find userspace FW,use embedded FW(size:%zu) instead",
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			if (g_ts_dbg != 0) {
				I("g_target_report_data->x[i]=%d,y[i]=%d,w[i]=%d",
...
/*zhangchaofan@ODM_LQ@Multimedia.TP,2019/11/05,add for add tp info node begin*/
#ifdef ODM_LQ_EDIT
	register_device_proc("tp", ic_data->vendor_ic_fw_ver, "hx83112a");
#endif
/*zhangchaofan@ODM_LQ@Multimedia.TP,2019/11/05,add for add tp info node end*/
