...
enum HX_INSPECT_ERR_ENUM {
	...
	/* Get raw data errors */
	HX_INSPECT_EGETRAW = 1 << 1,

	/* Memory allocate errors */
	HX_INSPECT_MEMALLCTFAIL = 1 << 2,

	/* Switch mode error*/
	HX_INSPECT_ESWITCHMODE = 1 << 3,

	/* Sensor open error */
	HX_EOPEN = 1 << (HIMAX_OPEN + ERR_SFT),

	/* Sensor micro open error */
	HX_EMOPEN = 1 << (HIMAX_MICRO_OPEN + ERR_SFT),

	/* Sensor short error */
	HX_ESHORT = 1 << (HIMAX_SHORT + ERR_SFT),

	/* Raw data error */
	HX_ERAW = 1 << (HIMAX_RAWDATA + ERR_SFT),

	/* Raw data BPN error */
	HX_EBPNRAW = 1 << (HIMAX_BPN_RAWDATA + ERR_SFT),

	/* Get SC errors */
	HX_ESC = 1 << (HIMAX_SC + ERR_SFT),

	/* Noise error */
	HX_WT_ENOISE = 1 << (HIMAX_WEIGHT_NOISE + ERR_SFT),

	/* Noise error */
	HX_ABS_ENOISE = 1 << (HIMAX_ABS_NOISE + ERR_SFT),

	/* Sorting error*/
	HX_ESORT = 1 << (HIMAX_SORTING + ERR_SFT),

	/* Raw Data GAP  */
	HX_EGAP_RAW = 1 << (HIMAX_GAPTEST_RAW + ERR_SFT),

	/* ACT_IDLE RAW ERROR */
	HX_EACT_IDLE_RAW = 1 << (HIMAX_ACT_IDLE_RAWDATA + ERR_SFT),

	/* ACT_IDLE NOISE ERROR */
	HX_EACT_IDLE_NOISE = 1 << (HIMAX_ACT_IDLE_NOISE + ERR_SFT),

	/* LPWUG RAW ERROR */
	HX_ELPWUG_RAW = 1 << (HIMAX_LPWUG_RAWDATA + ERR_SFT),

	/* LPWUG NOISE ERROR */
	HX_ELPWUG_WT_NOISE = 1 << (HIMAX_LPWUG_WEIGHT_NOISE + ERR_SFT),

	/* LPWUG NOISE ERROR */
	HX_ELPWUG_ABS_NOISE = 1 << (HIMAX_LPWUG_ABS_NOISE + ERR_SFT),

	/* LPWUG IDLE RAW ERROR */
	HX_ELPWUG_IDLE_RAW = 1 << (HIMAX_LPWUG_IDLE_RAWDATA + ERR_SFT),

	/* LPWUG IDLE NOISE ERROR */
	HX_ELPWUG_IDLE_NOISE = 1 << (HIMAX_LPWUG_IDLE_NOISE + ERR_SFT),
