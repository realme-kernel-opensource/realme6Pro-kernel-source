...
#define TPD_DEVICE "LQTP-interface"
...
int touch_i2c_continue_read(struct i2c_client *client, unsigned short length, unsigned char *data)
{
    ...
    if (retry == MAX_I2C_RETRY_TIME) {
        TPD_INFO("I2C read over retry limit\n");
        retval = -EIO;
    }
    return retval;
...
int touch_i2c_read_block(struct i2c_client *client, u16 addr, unsigned short length, unsigned char *data)
{
    ...
    if (retry == MAX_I2C_RETRY_TIME) {
        dev_err(&client->dev, "I2C read over retry limit\n");
        retval = -EIO;
    }
    return retval;
}
...
int touch_i2c_continue_write(struct i2c_client *client, unsigned short length, unsigned char *data)
{
    ...
    if (retry == MAX_I2C_RETRY_TIME) {
        TPD_INFO("I2C write over retry limit\n");
        retval = -EIO;
    }
    return retval;
}
...
int touch_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    ...
    if (readlen > 0) {
        ...
        if (retry == MAX_I2C_RETRY_TIME) {
            TPD_INFO("i2c_transfer(read) over retry limit\n");
            retval = -EIO;
        }
    }
...
int touch_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
    ...
    if (writelen > 0) {
        ...
        if (retry == MAX_I2C_RETRY_TIME) {
            TPD_INFO("i2c_transfer(write) over retry limit\n");
            retval = -EIO;
        }
    }
...
int32_t CTP_SPI_READ(struct spi_device *client, uint8_t *buf, uint16_t len)
{
    int32_t ret = -1;
    int32_t retries = 0;
	/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp read buf 2019-12-02 */
    uint8_t rbuf[SPI_READ_LEN + 1] = {0};
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp read buf 2019-12-02 */
