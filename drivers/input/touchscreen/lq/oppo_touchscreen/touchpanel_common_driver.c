...
#define TPD_DEVICE "LQTP-touchpanel"
#define TPD_INFO(fmt, arg...)	pr_info("[%s] %s %d: " fmt, TPD_DEVICE, __func__, __LINE__, ##arg)
#define TPD_DEBUG(fmt, arg...)\
    do{\
        if (LEVEL_DEBUG == tp_debug)\
            pr_err("[%s] %s %d: " fmt, TPD_DEVICE, __func__, __LINE__, ##arg);\
    }while(0)

#define TPD_DETAIL(fmt, arg...)\
    do{\
        if (LEVEL_BASIC != tp_debug)\
            pr_err("[%s] %s %d: " fmt, TPD_DEVICE, __func__, __LINE__, ##arg);\
    }while(0)

#define TPD_SPECIFIC_PRINT(count, fmt, arg...)\
    do{\
        if (count++ == TPD_PRINT_POINT_NUM || LEVEL_DEBUG == tp_debug) {\
            TPD_INFO("[%s] %s %d: " fmt, TPD_DEVICE, __func__, __LINE__, ##arg);\
            count = 0;\
        }\
    }while(0)
...
#ifdef ODM_LQ_EDIT
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp mode set 2020-01-02*/
static bool operate_switch_enable = false;
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp mode set 2020-01-02 */
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp vendor flag 2020-01-03*/
extern int g_tp_dev_vendor;
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp vendor flag 2020-01-03 */
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, add tp esd flag 2020-01-03*/
extern bool tp_gesture_esd_flag;
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, add tp esd flag 2020-01-03 */
#endif /*ODM_LQ_EDIT*/
...
static void tp_async_work_callback(void)
{
...
#endif
    {
        TPD_INFO("in ftm mode, no need to call back\n");
        return;
    }
    ...
    if (ts->in_test_process) {
        TPD_INFO("In test process, do not switch mode\n");
        return;
    }
...
static void tp_fw_update_work(struct work_struct *work)
{
    ...
    if (ts->ts_ops->fw_update) {
        ...
        if (!ret || ts->is_noflash_ic) {
            ...
            TPD_INFO("fw_name request failed %s %d\n", ts->panel_data.fw_name, ret);
            goto EXIT;
        }
    }
...
static void tp_freq_hop_work(struct work_struct *work)
{
    ...
    if (!ts->is_suspended) {
        ...
        if (!ts->ts_ops->freq_hop_trigger) {
            TPD_INFO("not support ts_ops->freq_hop_trigger callback\n");
            return;
        }
...
void switch_usb_state(int usb_state)
{
    ...
    if (g_tp->charger_pump_support && (g_tp->is_usb_checked != usb_state)) {
        ...
        TPD_INFO("check usb state : %d, is_suspended: %d\n", usb_state, g_tp->is_suspended);

#ifdef ODM_LQ_EDIT
		/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp mode set 2020-01-02*/
        if (!g_tp->is_suspended &&
				operate_switch_enable &&
...
void switch_headset_state(int headset_state)
{
    ...
    if (g_tp->headset_pump_support && (g_tp->is_headset_checked != headset_state)) {
        ...
        TPD_INFO("check headset state : %d, is_suspended: %d\n", headset_state, g_tp->is_suspended);
...
static ssize_t proc_game_switch_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TPD_INFO("game_switch value=0x%x\n", value);
    if (!ts->is_suspended) {
        mutex_lock(&ts->mutex);
        ts->ts_ops->mode_switch(ts->chip_data, MODE_GAME, value > 0);
        mutex_unlock(&ts->mutex);
    } else {
        TPD_INFO("game_switch_support is_suspended.\n");
    }
...
static ssize_t proc_irq_status_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    sscanf(buf, "%d", &value);
    TPD_INFO("%d, %s ts->irq=%d\n", value, value ? "enable" : "disable", ts->irq);
...
static ssize_t proc_glove_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    sscanf(buf, "%d", &ret);
    TPD_INFO("buf = %d, ret = %d\n", *buf, ret);
    if ((ret == 0) || (ret == 1)) {
        mutex_lock(&ts->mutex);
        ts->glove_enable = ret;
        ret = ts->ts_ops->mode_switch(ts->chip_data, MODE_GLOVE, ts->glove_enable);
        if (ret < 0) {
            TPD_INFO("Touchpanel operate mode switch failed\n");
        }
        mutex_unlock(&ts->mutex);
    }
    switch(ret) {
...
static ssize_t proc_limit_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TPD_INFO("limit_enable = 0x%x, ts->limit_edge = 0x%x, ts->limit_corner=0x%x\n", ts->limit_enable, ts->limit_edge, ts->limit_corner);
...
#ifdef ODM_LQ_EDIT
	/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp mode set 2020-01-02*/
	if (!operate_switch_enable) {
		operate_switch_enable = true;
	}
	/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp mode set 2020-01-02 */
#endif /*ODM_LQ_EDIT*/
...
static ssize_t proc_finger_protect_result_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if(ts->spuri_fp_touch.fp_touch_st == FINGER_PROTECT_TOUCH_UP || ts->spuri_fp_touch.fp_touch_st == FINGER_PROTECT_TOUCH_DOWN)
        TPD_INFO("report_finger_protect = %d\n", ts->spuri_fp_touch.fp_touch_st);
    ret = snprintf(page, PAGESIZE - 1, "%d\n", ts->spuri_fp_touch.fp_touch_st);
    ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
    return ret;
}
...
static int finger_protect_handler(void *data)
{
    ...
    do {
        ...
        if (ts->view_area_touched) {
            TPD_INFO("tp touch down,clear flag\n");
            ts->view_area_touched = 0;
        }
        operate_mode_switch(ts);
        mutex_unlock(&ts->mutex);
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
    ...
    if (prEntry_tp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("Couldn't create TP proc entry\n");
    }
...
static ssize_t proc_freq_hop_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TPD_INFO("freq_hop_simulate value=0x%x\n", value);
    if (!ts->is_suspended) {
        mutex_lock(&ts->mutex);
        tp_freq_hop_simulate(ts, value);
        mutex_unlock(&ts->mutex);
    } else {
        TPD_INFO("freq_hop_simulate is_suspended.\n");
        ts->freq_hop_info.freq_hop_freq = value;
    }
...
static ssize_t proc_earsense_palm_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TPD_DEBUG("value: %d, palm_enable :%d\n", value, ts->palm_enable);
...
static ssize_t proc_earsense_rawdata_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if (count != read_len) {
        TPD_INFO("length:%d not match data_len:%d\n", (int)count, read_len);
        return 0;
    }
...
static ssize_t proc_fp_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TPD_DETAIL("value: %d, fp_enable :%d\n", value, ts->fp_enable);
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
    ...
    if (prEntry_debug_info == NULL) {
        ret = -ENOMEM;
        TPD_INFO("Couldn't create debug_info proc entry\n");
    }
...
static int init_input_device(struct touchpanel_data *ts)
{
    ...
    if (input_register_device(ts->input_dev)) {
        TPD_INFO("Failed to register input device\n");
        input_free_device(ts->input_dev);
        return -1;
    }

    if (input_register_device(ts->kpd_input_dev)) {
        TPD_INFO("Failed to register key input device\n");
    ...
    if (ts->face_detect_support) {
        if (input_register_device(ts->ps_input_dev)) {
            TPD_INFO("Failed to register ps input device\n");
...
/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, remove invalid dts 2019-12-09 */
...
#ifdef ODM_LQ_EDIT
	/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp tx-rx num 2020-01-03 */
	if (g_tp_dev_vendor == TP_BOE) { //boe== 18 36
		rc = of_property_read_u32_array(np, "boe-touchpanel,tx-rx-num", tx_rx_num, 2);
	} else if (g_tp_dev_vendor == TP_JDI){   //jdi==16 36
		rc = of_property_read_u32_array(np, "jdi-touchpanel,tx-rx-num", tx_rx_num, 2);
	}
	...
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp tx-rx num 2020-01-03 */
#endif /*ODM_LQ_EDIT*/
...
#endif
/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, remove invalid dts 2019-12-09 */
#endif /*ODM_LQ_EDIT*/
...
int init_power_control(struct touchpanel_data *ts)
{
    ...
    // vddio 1.8v
    ts->hw_res.pwr_vdd = regulator_get(ts->dev, "touch_vddio");
    if (IS_ERR_OR_NULL(ts->hw_res.pwr_vdd)) {
		ret = PTR_ERR(ts->hw_res.pwr_vdd);
        TPD_INFO("Regulator get failed touch_vddio, ret = %d\n", ret);
		goto regulator_vcc_1v8_put;
    } else {
        if (regulator_count_voltages(ts->hw_res.pwr_vdd) > 0) {
            ret = regulator_set_voltage(ts->hw_res.pwr_vdd,
										NVT_SPI_VTG_MIN_UV,
										NVT_SPI_VTG_MAX_UV);
            if (ret) {
                dev_err(ts->dev, "Regulator set_vtg failed pwr_vdd rc = %d\n", ret);
                goto regulator_vcc_1v8_put;
            }
        }
    }
    ...
    // VSP +5V
    ts->hw_res.pwr_lab = regulator_get(ts->dev, "touch_lab");
    if (IS_ERR_OR_NULL(ts->hw_res.pwr_lab)) {
		ret = PTR_ERR(ts->hw_res.pwr_lab);
        TPD_INFO("Regulator get failed touch_lab, ret = %d\n", ret);
		goto regulator_lab_5v5_put;
    } else {
        if (regulator_count_voltages(ts->hw_res.pwr_lab) > 0) {
            ret = regulator_set_voltage(ts->hw_res.pwr_lab,
										NVT_lab_VTG_MIN_UV,
										NVT_lab_VTG_MAX_UV);
            if (ret) {
                dev_err(ts->dev, "Regulator set_vtg failed pwr_lab rc = %d\n", ret);
                goto regulator_lab_5v5_put;
            }
        }
    }
	...
	// VSN -5V
    ts->hw_res.pwr_ibb = regulator_get(ts->dev, "touch_ibb");
    if (IS_ERR_OR_NULL(ts->hw_res.pwr_ibb)) {
		ret = PTR_ERR(ts->hw_res.pwr_ibb);
        TPD_INFO("Regulator get failed touch_ibb, ret = %d\n", ret);
		goto regulator_ibb_5v5_put;
    } else {
        if (regulator_count_voltages(ts->hw_res.pwr_ibb) > 0) {
            ret = regulator_set_voltage(ts->hw_res.pwr_ibb,
										NVT_ibb_VTG_MIN_UV,
										NVT_ibb_VTG_MAX_UV);
            if (ret) {
                dev_err(ts->dev, "Regulator set_vtg failed pwr_ibb rc = %d\n", ret);
                goto regulator_ibb_5v5_put;
            }
        }
    }
...
regulator_ibb_5v5_put:
    if (!IS_ERR_OR_NULL(ts->hw_res.pwr_ibb)) {
        regulator_put(ts->hw_res.pwr_ibb);
        ts->hw_res.pwr_ibb = NULL;
	}
regulator_lab_5v5_put:
	if (!IS_ERR_OR_NULL(ts->hw_res.pwr_lab)) {
		regulator_put(ts->hw_res.pwr_lab);
		ts->hw_res.pwr_lab = NULL;
	}
regulator_vcc_1v8_put:
	if (!IS_ERR_OR_NULL(ts->hw_res.pwr_vdd)) {
		regulator_put(ts->hw_res.pwr_vdd);
		ts->hw_res.pwr_vdd = NULL;
	}
...
int tp_powercontrol_enable(struct hw_resource *hw_res, bool on)
{
    int ret = 0;

    if (on) {// power on
		if (hw_res->pwr_vdd) {
			ret = regulator_enable(hw_res->pwr_vdd);
			if (ret < 0) {
				TPD_INFO("Failed to enable vdd regulator");
				goto disable_vdd_regulator;
			}
		}
		if (hw_res->pwr_lab) {
			ret = regulator_enable(hw_res->pwr_lab);
			if (ret < 0) {
				TPD_INFO("Failed to enable lab regulator");
				goto disable_lab_regulator;
			}
		}

		if (hw_res->pwr_ibb) {
			ret = regulator_enable(hw_res->pwr_ibb);
			if (ret < 0) {
				TPD_INFO("Failed to enable ibb regulator");
				goto disable_ibb_regulator;
			}
		}
    } else {// power off
        if (!IS_ERR_OR_NULL(hw_res->pwr_ibb)) {
            ret = regulator_disable(hw_res->pwr_ibb);
            if (ret) {
                TPD_INFO("Regulator pwr_ibb enable failed rc = %d\n", ret);
                return ret;
            }
        }

		if (!IS_ERR_OR_NULL(hw_res->pwr_lab)) {
            ret = regulator_disable(hw_res->pwr_lab);
            if (ret) {
                TPD_INFO("Regulator pwr_lab enable failed rc = %d\n", ret);
                return ret;
            }
        }

		if (!IS_ERR_OR_NULL(hw_res->pwr_vdd)) {
            ret = regulator_disable(hw_res->pwr_vdd);
            if (ret) {
                TPD_INFO("Regulator pwr_vdd enable failed rc = %d\n", ret);
                return ret;
            }
        }
    }
...
disable_ibb_regulator:
	if (hw_res->pwr_ibb)
		regulator_disable(hw_res->pwr_ibb);
disable_lab_regulator:
	if (hw_res->pwr_lab)
		regulator_disable(hw_res->pwr_lab);
disable_vdd_regulator:
	if (hw_res->pwr_vdd)
		regulator_disable(hw_res->pwr_vdd);
...
int tp_register_irq_func(struct touchpanel_data *ts)
{
    ...
    if (gpio_is_valid(ts->hw_res.irq_gpio)) {
        ...
        ret = request_threaded_irq(ts->irq, NULL,
                                   ...
                                   TPD_DEVICE, ts);
        if (ret < 0) {
            TPD_INFO("request_threaded_irq ret is %d\n", ret);
        }
    } else {
        TPD_INFO("no valid irq\n");
    }
#else
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
    ...
    if (ret) {
        TPD_INFO("tp power init failed.\n");
        return -1;
    }
    ret = reconfig_power_control(ts);
    if (ret) {
        TPD_INFO("reconfig power failed.\n");
        return -1;
    }
...
#endif
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for remove tp init power 2019-12-26 */
#endif /*ODM_LQ_EDIT*/

    //step5 : I2C function check
    if (!ts->is_noflash_ic) {
        if (!i2c_check_functionality(ts->client->adapter, I2C_FUNC_I2C)) {
            TPD_INFO("need I2C_FUNC_I2C\n");
            ret = -ENODEV;
...
/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP for read lcd vendor id, 2019-11-18 */
#ifdef ODM_LQ_EDIT
...
#endif
    {
        ...
        TPD_INFO("not int normal mode, return.\n");
        return ret;
    }
   ...
   //     goto manu_info_alloc_err;
	...
	/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp fw delayed work 2020-01-03 */
    INIT_DELAYED_WORK(&ts->lcd_trigger_load_tp_fw_work, lcd_trigger_load_tp_fw);
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp fw delayed work 2020-01-03 */
...
#ifdef ODM_LQ_EDIT
/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, remove invalid charger_pump_support 2019-12-09 */
#if 0
...
#endif
/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, remove invalid charger_pump_support 2019-12-09 */
#endif /*ODM_LQ_EDIT*/
...
#ifdef ODM_LQ_EDIT
		/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp fw delayed work 2020-01-03*/
		if (tp_gesture_esd_flag)
			queue_delayed_work(g_tp->lcd_trigger_load_tp_fw_wq, &(g_tp->lcd_trigger_load_tp_fw_work), msecs_to_jiffies(130));
		else
			queue_delayed_work(g_tp->lcd_trigger_load_tp_fw_wq, &(g_tp->lcd_trigger_load_tp_fw_work), 0);
		/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp fw delayed work 2019-01-03 */
#endif /*ODM_LQ_EDIT*/
    }
}
...
{
    struct touchpanel_data *ts = container_of(work, struct touchpanel_data,
                                 lcd_trigger_load_tp_fw_work.work);
    static bool is_running = false;

    if (ts->lcd_trigger_load_tp_fw_support) {
        if (is_running) {
            TPD_INFO("is running, can not repeat\n");
        } else {
            TPD_INFO("start\n");
            is_running = true;
            mutex_lock(&ts->mutex);
...
void tp_i2c_suspend(struct touchpanel_data *ts)
{
    ...
    if (ts->black_gesture_support || ts->fingerprint_underscreen_support) {
        if ((ts->gesture_enable & 0x01) == 1 || ts->fp_enable == 1) {
            /*enable gpio wake system through interrupt*/
            enable_irq_wake(ts->irq);
			TPD_INFO("is called ,ts->new_set_irq_wake_support = %d \n",ts->new_set_irq_wake_support);
...
{
    if (g_tp) {
        TPD_INFO("common panel struct has alloc already!\n");
        return NULL;
    }
