...
#define TPD_LOG "LQTP-nt36672c"
#define TPD_INFO(fmt, arg...)	pr_info("[%s]   %s %d: " fmt, TPD_LOG, __func__, __LINE__, ##arg)
#define TPD_DEBUG(fmt, arg...)\
    do{\
        if (LEVEL_DEBUG == tp_debug)\
            pr_err("[%s] %s %d: " fmt, TPD_LOG, __func__, __LINE__, ##arg);\
    }while(0)
...
#ifdef ODM_LQ_EDIT
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP for read lcd vendor id, 2019-12-02*/
...
#define CP_TP_CPU_REQ       0x3F291	//modify by zhangchaofan@ODM_LQ@Multimedia.TP, 2019-12-05
...
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP for read lcd vendor id, 2019-12-02*/
#endif
...
static int nvt_enter_sleep(struct chip_data_nt36672c *chip_info, bool config)
{
    ...
    if (config) {
        ...
        if (ret < 0) {
            ...
            TPD_INFO("enter sleep mode sucess!\n");
        }
    }
...
static int32_t Download_Init(struct chip_data_nt36672c *chip_info)
{
    ...
    if (chip_info->fwbuf == NULL) {
		/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for upload fw buf 2019-12-02 */
        chip_info->fwbuf = (uint8_t *)kzalloc((SPI_TANSFER_LEN + 1 + DUMMY_BYTES), GFP_KERNEL);
		/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for upload fw buf 2019-12-02 */
        if(chip_info->fwbuf == NULL) {
...
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for display esd recovery 2019-12-02*/
#if NVT_TOUCH_ESD_DISP_RECOVERY
...
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for display esd recovery 2019-12-02 */
            break;
        }
    }
...
//	ret = tp_powercontrol_enable(chip_info->hw_res, enable);
//	if (ret)
//		return -1;
...
static int nvt_get_vendor(void *chip_data, struct panel_info *panel_data)
{
    ...
    if ((len > 3) && (panel_data->fw_name[len - 3] == 'i') && \
		...
		//TPD_INFO("panel_data->fw_name[len - 3] = %s\n", panel_data->fw_name[len - 3]);
    }
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    ret = CTP_SPI_READ(chip_info->s_client, point_data, POINT_DATA_LEN + 1);
    if (ret < 0) {
        TPD_INFO("read gesture data failed\n");
        return -1;
    }
    ...
    TPD_INFO("gesture_id: 0x%x, func_type: 0x%x, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n", \
...
static __maybe_unused int nvt_enable_debug_gesture_coordinate_record_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
    ...
    TPD_INFO("enable = %d, chip_info->is_sleep_writed = %d\n",
...
static uint8_t edge_limit_level = 40; /* 0 ~ 255 modify by zhangchaofan@ODM_LQ@Multimedia.TP, 2019-12-02 */
static int nvt_enable_edge_limit(struct chip_data_nt36672c *chip_info, int state)
{
    int8_t ret = -1;
    struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
    TPD_INFO("state = %d, limit_corner = %d, level = %d, chip_info->is_sleep_writed = %d\n", state, ts->limit_corner, edge_limit_level, chip_info->is_sleep_writed);
...
static int nvt_enable_headset_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
    ...
    TPD_DEBUG("enable = %d, chip_info->is_sleep_writed = %d\n",
              enable, chip_info->is_sleep_writed);

    if (enable) {
        ret = nvt_cmd_store(chip_info, EVENTBUFFER_HS_PLUG_IN);
        TPD_INFO("EVENTBUFFER_HS_PLUG_IN\n");
    } else {
...
static int nvt_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    case MODE_SLEEP:
        ret = nvt_enter_sleep(chip_info, true);
        if (ret < 0) {
            TPD_INFO("nvt enter sleep failed\n");
        }
    ...
    case MODE_GESTURE:
        ...
        if (ret < 0) {
            TPD_INFO("nvt enable gesture failed.\n");
            return ret;
        }
    ...
    case MODE_EDGE:
        ...
        if (ret < 0) {
            TPD_INFO("nvt enable edg limit failed.\n");
            return ret;
        }
        break;
    ...
    case MODE_CHARGE:
        ...
        if (ret < 0) {
            TPD_INFO("enable charge mode : %d failed\n", flag);
        }
        break;
    ...
    case MODE_HEADSET:
        ...
        if (ret < 0) {
            TPD_INFO("enable headset mode : %d failed\n", flag);
        }
        break;
    ...
    /*case MODE_HOPPING_POLLING:	//TODO: add define in oppo h file, MODE_HOPPING_POLLING
    	...
    	if (ret < 0) {
    		TPD_INFO("enable hopping polling mode : %d failed\n", flag);
    	}
    	break;
    ...
    case MODE_HOPPING_FIX_FREQ:	//TODO: add define in oppo h file, MODE_HOPPING_FIX_FREQ
    	...
    	if (ret < 0) {
    		TPD_INFO("enable hopping fix freq mode : %d failed\n", flag);
    	}
    	break;*/
    ...
    /*case DEBUG_MODE_MESSAGE_DIFF:	//TODO: add define in oppo h file, DEBUG_MODE_MESSAGE_DIFF
    	...
    	if (ret < 0) {
    		TPD_INFO("enable debug message diff %d failed\n", flag);
    	}
    	break;
    ...
    case DEBUG_MODE_GESTURE_COORDINATE:	//TODO: add define in oppo h file, DEBUG_MODE_GESTURE_COORDINATE
    	...
    	if (ret < 0) {
    		TPD_INFO("enable debug gesture coordinate %d failed\n", flag);
    	}
    	break;
    ...
    case DEBUG_MODE_GESTURE_COORDINATE_RECORD:	//TODO: add define in oppo h file, DEBUG_MODE_GESTURE_COORDINATE_RECORD
    	...
    	if (ret < 0) {
    		TPD_INFO("enable debug gesture coordinate record %d failed\n", flag);
    	}
    	break;*/
...
static fw_check_state nvt_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    if (ret < 0) {
        ...
        if (panel_data->manufacture_info.version) {
			...
			#ifdef ODM_LQ_EDIT
			/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp devinfo 2019-12-12 */
                ver_len = strlen(panel_data->manufacture_info.version);
				panel_data->vid_len = ver_len;
				strlcpy(&panel_data->manufacture_info.version[ver_len], dev_version, MAX_DEVICE_VERSION_LENGTH - 8);
			/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp devinfo 2019-12-12 */
			#endif /*ODM_LQ_EDIT*/
            } else {
...
static int32_t nvt_check_bin_checksum(const u8 *fwdata, size_t fwsize)
{
    ...
    if (checksum_calculated != checksum_bin) {
        TPD_INFO("checksum_calculated = 0x%08X\n", checksum_calculated);
        TPD_INFO("checksum_bin = 0x%08X\n", checksum_bin);
        ret = -EINVAL;
    }
    return ret;
}
...
OUT:
    ...
    snprintf(message, MESSAGE_SIZE, "%d errors. %s\n", err_cnt, buf);
...
static void nvt_auto_test(struct seq_file *s, void *chip_data, struct nvt_testdata *nvt_testdata)
{
    ...
    if(ret > 0) {
        TPD_INFO("fw update failed!\n");
		#ifdef ODM_LQ_EDIT
        /*qujiong@ODM_LQ@BSP.TP,2019/11/19,add for TP selftest*/
		err_cnt = 1;
		#endif
        goto RELEASE_FIRMWARE;
    }
...
/*******************************************************
Description:
Novatek touchscreen f2c read/write control function.
...
int nvt_f2c_read_write(void *chip_data, uint8_t F2C_RW, uint32_t DDIC_REG_ADDR, uint16_t len, uint8_t *data)
{
	uint8_t buf[8] = {0};
	int ret = 0;
	int32_t f2c_retry = 0;
	uint8_t retry = 0;
	uint32_t retry_max = 1000;	//modify by zhangchaofan@ODM_LQ@Multimedia.TP, 2019-12-05
...
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp CP_TP_CPU_REQ 2019-12-05*/
	//Enable CP_TP_CPU_REQ
	nvt_write_addr(chip_info->s_client, CP_TP_CPU_REQ, 1);
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp CP_TP_CPU_REQ 2019-12-05 */
...
nvt_f2c_retry:
	...
	/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, 2019-12-05 */
		if(unlikely(retry > 20)) {
			TPD_INFO("Wait F2C_EN = 0 failed! retry %d\n", retry);
			return -EIO;
		}
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, 2019-12-05 */
	}
	//Step7: Check REG TH_CPU_CHK  status (1: Success,  0: Fail), if 0, can Retry Step5.
	//TPD_INFO("f2c_retry=%d, buf[1]=0x%02X, buf[2]=0x%02X\n", f2c_retry, buf[1], buf[2]);
...
static int nvt_tp_probe(struct spi_device *client)
{
    ...
    ts->s_client = client;
	//ts->s_client->chip_select = 0;   //add for recover all TP module
    ts->irq = client->irq;
    ...
    if (ts->esd_handle_support) {
        ...
        TPD_INFO("change esd handle time to %d s\n", ts->esd_info.esd_work_time / HZ);
    }
...
static void nvt_tp_shutdown(struct spi_device *client)
{
	struct touchpanel_data *ts = spi_get_drvdata(client);
