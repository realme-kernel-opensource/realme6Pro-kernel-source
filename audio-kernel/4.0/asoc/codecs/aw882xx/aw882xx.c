...
static void aw882xx_reg_loaded(const struct firmware *cont, void *context)
{
	...
	if (ret < 0) {
		...
		//chengong@ODM_LQ@Multimedia.Audio,2019/11/22,add for cali
		if (aw882xx_cali_re >= R0_MIN && aw882xx_cali_re <= R0_MAX) {
			cali_re = aw882xx_cali_re;
			printk("%s cali_re %d\n", __func__, cali_re);
		} else
		...
		if (cali_re < R0_MIN || cali_re > R0_MAX) {
			cali_re = DEFAULT_CALI_VALUE * 1000;
			printk("%s use default vaule %d\n", __func__, cali_re);
		}
		cali_re = (cali_re << 12) / 1000;
...
static int aw882xx_cali_re_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = aw882xx_cali_re;

	return 0;
}

static int aw882xx_cali_re_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	aw882xx_cali_re = ucontrol->value.integer.value[0];

	pr_debug("%s: cali_re=%d\n", __func__, aw882xx_cali_re);
...
static struct snd_kcontrol_new aw882xx_controls[] = {
	...
	//chengong@ODM_LQ@Multimedia.Audio,2019/11/22,add for cali
	SOC_SINGLE_EXT("aw882xx_cali_re", SND_SOC_NOPM, 0, 10000, 0,
			   aw882xx_cali_re_get, aw882xx_cali_re_put),
};
